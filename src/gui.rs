...
impl I18n {
    ...
    fn invalid_ip(&self) -> &str {
        match self.language {
            Language::Spanish => "Dirección de servidor no válida",
            Language::English => "Invalid server address",
        }
    }
...
impl ConfigApp {
    pub fn new(config: Arc<Mutex<Config>>) -> Self {
        ...
        drop(cfg); // Liberar el lock explícitamente
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button(self.i18n.save()).clicked() {
                    // Validar IP (básicamente que no esté vacía)
